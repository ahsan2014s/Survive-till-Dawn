...
def spawn_ranged_enemy():
    enemy = {
        ...
        'base_speed': 0.0,  # they don’t chase
        'state': 'ALIVE',
        'death_time': 0,
        'health': 40.0,
        'max_health': 40.0,
...
def update_projectiles(dt):
    ...
    for p in ranged_projectiles:
        p['pos'] += p['vel'] * dt
        if abs(p['pos'][0]) < grid_size and abs(p['pos'][2]) < grid_size:
            # check collision with player
            if np.linalg.norm(player_pos[:2] - p['pos'][:2]) < 1.0:
                player_health -= 10
            else:
                new_proj.append(p)
    ranged_projectiles[:] = new_proj


def fire_projectile(ex, ez):
    dx = player_pos[0] - ex
    dz = player_pos[2] - ez
    d = math.hypot(dx, dz)
    if d == 0: return
    ...
    ranged_projectiles.append({
        'pos': np.array([ex, 1.0, ez]),
        'vel': np.array([dx/d * speed, 0, dz/d * speed])
    })
...
def draw_healing_item():
    if healing_item:
        glPushMatrix()
        glTranslatef(healing_item[0], 0.5, healing_item[1])
        glColor3f(0.0, 1.0, 0.0)
...
def draw_projectiles():
    for p in ranged_projectiles:
        ...
        glColor3f(1.0, 0.0, 0.0)  # red projectile
        glutSolidSphere(0.3, 10, 10)
        glPopMatrix()
...
def draw_obstacles():
    for ox, oz in obstacles:
        ...
        glColor3f(0.4, 0.2, 0.1)  # brown cube
        glutSolidCube(1.0)
        glPopMatrix()
...
def draw_healing_item():
    if healing_item:
        glPushMatrix()
        glTranslatef(healing_item[0], 0.5, healing_item[1])
        glColor3f(0.0, 1.0, 0.0)  # green glowing sphere
    ...
    for e in ranged_enemies:
      if e['state'] == 'ALIVE':
        ...
        if e['cooldown'] <= 0:
            ...
            e['cooldown'] = 4.0  # shoot every 4 seconds
...
def check_collisions():
    global healing_item, game_score, player_health, bullets, heal_items
    if is_game_over: 
        return
    ...
    if healing_item:
       if abs(player_pos[0] - healing_item[0]) < 1.0 and abs(player_pos[2] - healing_item[1]) < 1.0:
           player_health = min(MAX_PLAYER_HEALTH, player_health + HEAL_AMOUNT)
           healing_item = None
...
def game_loop():
    ...
    if not is_game_over:
        ...
        global healing_item, healing_timer 
        healing_timer += dt
        if healing_timer > random.uniform(20, 30):
           healing_timer = 0.0
           healing_item = [random.uniform(-grid_size+5, grid_size-5), random.uniform(-grid_size+5, grid_size-5)]
