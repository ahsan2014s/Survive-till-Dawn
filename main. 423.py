...
def draw_enemy(enemy):
    if enemy['state'] == 'ALIVE':
        ...
        glPopMatrix()
    if enemy in ranged_enemies and enemy.get('glow_time', 0) > 0:
        glPushMatrix()
        glTranslatef(enemy['pos'][0], enemy['pos'][1] + 1.0, enemy['pos'][2])
        glColor4f(1.0, 0.0, 0.0, 0.5)   # glowing aura
...
def spawn_ranged_enemy():
    enemy = {
        ...
        'rot_y': 0.0,
        'cooldown': 0.0,
        'glow_time': 0.0  # 🔴 flash when firing
    }
    respawn_enemy(enemy)
...
def spawn_ranged_enemy():
    ...
    enemy = {'pos': np.array([spawn_x, 1.0, spawn_z]), 'health': 3}
    ranged_enemies.append(enemy)
...
def check_collisions():
    ...
    if healing_item_active:
      dx = player_x - healing_item_x
      dz = player_z - healing_item_z
      distance = (dx*dx + dz*dz)**0.5
      if distance <= 1.5:  # adjust pickup range if needed
        life += 1
        healing_item_active = False
